...
plt.title('Distribution of Basic pKa Values')
plt.xlabel('pKa')
...
plt.axhline(y=mean_test_scoresrf[0], color='red', linestyle='--', label=f'y = {mean_test_scoresrf[0]}')
plt.grid(True)
plt.show()
...
# %%
errorsKNN = mean_test_scoresKNNRMSE + mean_test_scoresKNNMAE
...
plt.plot(n_neighbors_values, errorsKNN, marker='x', linestyle='--', color='black')

# %%
modelKNN2 = KNeighborsRegressor(n_neighbors=3)
modelKNN2.fit(Xdesc_train_scaled, Ydesc_trainKNN)

# %%
maescoresKNN2 = cross_val_score(modelKNN2, Xdesc_train_scaled, Ydesc_trainKNN, scoring='neg_mean_absolute_error', cv=5, n_jobs=-1)
rmsescoresKNN2 = cross_val_score(modelKNN2, Xdesc_train_scaled, Ydesc_trainKNN, scoring='neg_mean_squared_error', cv=5, n_jobs=-1)
r2_scoresKNN2 = cross_val_score(modelKNN2, Xdesc_train_scaled, Ydesc_trainKNN, scoring='r2', cv=5, n_jobs=-1)

# %%
rmse_scoresKNN2 = np.sqrt(-rmsescoresKNN2)
mae_scoresKNN2 = absolute(maescoresKNN2)

# %%
print(f'MAE: {mae_scoresKNN2.mean()},({mae_scoresKNN2.std()})')
print(f'RMSE: {rmse_scoresKNN2.mean()},({rmse_scoresKNN2.std()})')
print(f'R-squared: {r2_scoresKNN2.mean()}, ({r2_scoresKNN2.std()})')

# %%
predictionsKNN2 = modelKNN2.predict(Xdesc_test_scaled)

# %%
mseKNN2 = mean_squared_error(Ydesc_test, predictionsKNN2)
maeKNN2 = mean_absolute_error(Ydesc_test, predictionsKNN2)
rmseKNN2 = np.sqrt(mseKNN2)
r2KNN2 = r2_score(Ydesc_test, predictionsKNN2)
print(f'MAE: { maeKNN2}')
print(f'RMSE: {rmseKNN2}')
print(f'R-squared: {r2KNN2}')
...
# %%
Xdesc_train_scaled[:, mi_scores > 0.125].shape

# %%
maeKNN_values = []
rmseKNN_values = []
k_values = np.arange(0, 0.15, 0.0005)
for k in k_values:
...
modelKNNreduced.fit(Xdesc_train_scaled[:, mi_scores > 0.125], Ydesc_trainKNN)

# %%
maescoresKNNreduced = cross_val_score(modelKNNreduced, Xdesc_train_scaled[:, mi_scores > 0.125], Ydesc_trainKNN, scoring='neg_mean_absolute_error', cv=5, n_jobs=-1)
rmsescoresKNNreduced = cross_val_score(modelKNNreduced, Xdesc_train_scaled[:, mi_scores > 0.125], Ydesc_trainKNN, scoring='neg_mean_squared_error', cv=5, n_jobs=-1)
r2_scoresKNNreduced = cross_val_score(modelKNNreduced, Xdesc_train_scaled[:, mi_scores > 0.125], Ydesc_trainKNN, scoring='r2', cv=5, n_jobs=-1)
...
# %%
predictionsKNNreduced = modelKNNreduced.predict(Xdesc_test_scaled[:, mi_scores > 0.125])
...
# %%
correlation_coefficient, p_valuecc = pearsonr(predictionsrf2, Ydesc_test_array)
...
absolute_errorsKNN = [abs(actual - predicted) for actual, predicted in zip(Ydesc_test_array, predictionsKNN2)]
